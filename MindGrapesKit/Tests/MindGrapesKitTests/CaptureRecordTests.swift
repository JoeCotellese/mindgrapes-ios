...
/// Every test that builds a `ModelContainer`, run one at a time.
///
/// Constructing containers concurrently segfaults inside CoreData's schema
/// setup (`_generateTriggerSQL` mutating a shared dictionary), which showed up
/// as a roughly 1-in-10 `SIGSEGV` under Swift Testing's default parallelism.
/// The app only ever builds one container per process, so this is a test-host
/// constraint, not a product defect: anything added here that opens a store
/// must stay inside this suite.
@Suite(.serialized)
struct CaptureRecordTests {
    ...
    @Test func peopleComeBackFromTheStoreTrimmedAndNamed() throws {
        // `people` is a Codable composite attribute, so this read runs
        // `Person.init(from:)`. The assertion is that the store hands back the
        // same validated value the pipeline put in, not the raw input.
        let store = try TemporaryStore()
        let draft = try #require(
            NoteDraft(
                content: "Dinner with the neighbours.",
                people: [Person(name: "  Anna \n", relationship: " neighbour ")].compactMap { $0 }
            )
        )

        let writeContext = ModelContext(store.container)
        writeContext.insert(CaptureRecord(note: draft))
        try writeContext.save()

        let record = try #require(
            try ModelContext(store.container).fetch(FetchDescriptor<CaptureRecord>()).first
        )
        #expect(record.people.count == 1)
        #expect(record.people.first?.name == "Anna")
        #expect(record.people.first?.relationship == "neighbour")
    }
